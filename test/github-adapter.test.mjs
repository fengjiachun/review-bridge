...
test("a quoted or fenced trigger phrase is not a review request", async () => {
  const input = await fixture("codex-clean");
  input.request_history = [];
  const quoted = structuredClone(input.issue_comments[0]);
  quoted.id += 1;
  quoted.body = "The App's help text says\n\n> Comment \"@codex review\".\n";
  const fenced = structuredClone(input.issue_comments[0]);
  fenced.id += 2;
  fenced.body = "The trigger is\n\n```\n@codex review\n```\n";
  ...
  const splitByFence = structuredClone(input.issue_comments[0]);
  splitByFence.id += 11;
  splitByFence.body = "@codex\n\n```\nnote\n```\n\nreview\n";
  const splitByQuote = structuredClone(input.issue_comments[0]);
  splitByQuote.id += 12;
  splitByQuote.body = "@codex\n\n> note\n\nreview\n";
  const bareCr = structuredClone(input.issue_comments[0]);
  bareCr.id += 13;
  bareCr.body = "Example\r```\r@codex review\r```\r";
  const crlf = structuredClone(input.issue_comments[0]);
  crlf.id += 14;
  crlf.body = "Example\r\n```\r\n@codex review\r\n```\r\n";
  ...
  const tabFence = structuredClone(input.issue_comments[0]);
  tabFence.id += 7;
  tabFence.body = "\t```\n@codex review\n";
  const tabQuote = structuredClone(input.issue_comments[0]);
  tabQuote.id += 8;
  tabQuote.body = "\t> @codex review\n";
  const indentedFence = structuredClone(input.issue_comments[0]);
  indentedFence.id += 9;
  indentedFence.body = "   ```\n@codex review\n   ```\n";
  input.issue_comments.push(
    quoted,
  ...
  assert.deepEqual(
    ...
    [mentioned.id, backtickInfo.id, tabFence.id, tabQuote.id],
  );
