...
test("Hermes install and release artifacts use the 0.15.2 release identity", async () => {
  const [
    packageJson,
    packageLock,
    build,
  ...
  assert.match(hermesReadme, /exact `v0\.15\.2` tag/);
  ...
  assert.match(server, /version: "0\.15\.2"/);
  ...
  assert.match(rootReadme, /v0\.15\.2/);
  assert.match(hermesReadme, /v0\.15\.2/);
  assert.match(hermesAuthorConfig, /v0\.15\.2/);
  assert.match(hermesReviewerConfig, /v0\.15\.2/);
  assert.match(deepseekReadme, /exact `v0\.15\.2` tag/);
  assert.match(deepseekAuthorPatch, /v0\.15\.2/);
  assert.match(deepseekReviewerPatch, /v0\.15\.2/);
  ...
  assert.ok(currentChangelog, "the 0.15.2 changelog entry is missing");
