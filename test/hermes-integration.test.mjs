...
test("Hermes install and release artifacts use the 0.15.1 release identity", async () => {
  const [
    packageJson,
    packageLock,
    build,
  ...
  assert.match(hermesReadme, /exact `v0\.15\.1` tag/);
  ...
  assert.match(server, /version: "0\.15\.1"/);
  ...
  assert.match(rootReadme, /v0\.15\.1/);
  assert.match(hermesReadme, /v0\.15\.1/);
  assert.match(hermesAuthorConfig, /v0\.15\.1/);
  assert.match(hermesReviewerConfig, /v0\.15\.1/);
  assert.match(deepseekReadme, /exact `v0\.15\.1` tag/);
  assert.match(deepseekAuthorPatch, /v0\.15\.1/);
  assert.match(deepseekReviewerPatch, /v0\.15\.1/);
  ...
  assert.ok(currentChangelog, "the 0.15.1 changelog entry is missing");
