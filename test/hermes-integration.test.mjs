...
test("Hermes install and release artifacts use the 0.11.0 release identity", async () => {
  const [
    packageJson,
    packageLock,
    build,
  ...
  assert.match(hermesReadme, /exact `v0\.11\.0` tag/);
  ...
  assert.match(server, /version: "0\.11\.0"/);
  ...
  assert.match(rootReadme, /v0\.11\.0/);
  assert.match(hermesReadme, /v0\.11\.0/);
  assert.match(hermesAuthorConfig, /v0\.11\.0/);
  assert.match(hermesReviewerConfig, /v0\.11\.0/);
  assert.match(deepseekReadme, /exact `v0\.11\.0` tag/);
  assert.match(deepseekAuthorPatch, /v0\.11\.0/);
  assert.match(deepseekReviewerPatch, /v0\.11\.0/);
  ...
  assert.ok(currentChangelog, "the 0.11.0 changelog entry is missing");
  assert.match(currentChangelog.groups.body, /CODEX_TASK/);
