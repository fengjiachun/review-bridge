...
test("Hermes install and release artifacts use the 0.14.1 release identity", async () => {
  const [
    packageJson,
    packageLock,
    build,
  ...
  assert.match(hermesReadme, /exact `v0\.14\.1` tag/);
  ...
  assert.match(server, /version: "0\.14\.1"/);
  ...
  assert.match(rootReadme, /v0\.14\.1/);
  assert.match(hermesReadme, /v0\.14\.1/);
  assert.match(hermesAuthorConfig, /v0\.14\.1/);
  assert.match(hermesReviewerConfig, /v0\.14\.1/);
  assert.match(deepseekReadme, /exact `v0\.14\.1` tag/);
  assert.match(deepseekAuthorPatch, /v0\.14\.1/);
  assert.match(deepseekReviewerPatch, /v0\.14\.1/);
  ...
  assert.ok(currentChangelog, "the 0.14.1 changelog entry is missing");
