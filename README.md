...
reviewer against one store. The version examples below use `v0.11.0`; substitute
...
npm ci
npm run build
codex plugin marketplace add "$(pwd)/dist/review-bridge-v0.11.0/codex-marketplace"
```
...
absolute, versioned `review-bridge-v0.11.0/hermes-integration` directory and
...
absolute, versioned `review-bridge-v0.11.0/deepseek-harness` directory and
...
`npm run build` writes everything under `dist/review-bridge-v0.11.0/`:
...
- `review-bridge-reviewer-v0.11.0.mcpb` — MCP Bundle for Claude Desktop.
- `review-bridge-reviewer-v0.11.0.dxt` — compatibility copy of the same bundle.
...
- `deepseek-harness/` — DeepSeek Harness cordis patch snippets (separate author
  ...
  install/upgrade/isolation documentation, with the same packaged server
  runtime.
- `review-bridge-source-v0.11.0.zip` — source archive of the built commit, for
...
```bash
node dist/review-bridge-v0.11.0/codex-marketplace/plugins/review-bridge/scripts/collect-github-observation.mjs --review-id <review_id>
```
