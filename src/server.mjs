...
if (role === "author") {
  ...
  register(
    "render_review_report",
    {
      ...
      description:
        "Render a human-readable Markdown report of a review from its ledger and, when present, its publication ledger -- or, for a REMOTE_ONLY publication with no review ledger, from the publication and its authorization alone -- and write it to reviews/<review_id>/report-r<revision>.md (report-p<revision>.md when remote-only) beside them. Returns a receipt -- path, byte count, sha256, the ledger revisions rendered, and whether an existing file at that revision was reused -- never the Markdown itself, which can run to megabytes; read the file, or print the path for the operator. Read-only over the ledgers: it changes no review or publication state, consumes no round, and touches no gate. The report is a projection of the ledger, not evidence.",
      inputSchema: { review_id: z.string() },
    },
