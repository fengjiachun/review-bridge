...
if (role === "author") {
  ...
  register(
    "render_review_report",
    {
      ...
      description:
        "Render a human-readable Markdown report of a review from its ledger and, when present, its publication ledger -- or, for a REMOTE_ONLY publication with no review ledger, from the publication and its authorization alone -- and write it to reviews/<review_id>/report-r<state_version>[-p<revision>-s<summary digest>].md (report-p<revision>-s<summary digest>.md when remote-only) beside them; the summary digest covers the publication summary fields the report prints, so a gate appearing or evidence expiring writes a new report. Returns a receipt -- path, byte count, sha256, the ledger revisions rendered, and whether an existing file at that revision was reused -- never the Markdown itself, which can run to megabytes; read the file, or print the path for the operator. Read-only over the ledgers: it changes no review or publication state, consumes no round, and touches no gate. A review created before worktree_clean was recorded cannot be rendered, because the store cannot reproduce its snapshot hash (ROUND_SNAPSHOT_UNREPRODUCIBLE), and neither can a continuation prepared before the source freeze, whose source never recorded it (CONTINUATION_SOURCE_MISMATCH). The report is a projection of the ledger, not evidence.",
      inputSchema: { review_id: z.string() },
    },
