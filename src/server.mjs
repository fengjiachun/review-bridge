...
if (role === "author") {
  ...
  register(
    "record_automatic_resolution",
    {
      ...
      description:
        "Store the server-owned proof that this workflow performed the thread's unresolved-to-resolved transition. Every binding is recomputed from the publication's own evidence; the observation is cleared because the mutation outdated it. Idempotent for the same action.",
      inputSchema: {
        review_id: z.string(),
        expected_revision: z.number().int().positive(),
        workflow_id: z.string(),
        ...
        eligibility_sha256: z.string(),
        reply_comment_id: z.number().int().positive(),
        actor_id: z.number().int().positive(),
        actor_type: z.string(),
        pre_read_observed_at: z.string(),
        post_read_observed_at: z.string(),
        resolved_by_id: z.number().int().positive(),
        resolved_by_type: z.string(),
      },
    },
    (input) =>
      recordAutomaticResolution(
        ...
        input.review_id,
        {
          ...
          threadId: input.thread_id,
          threadWatermark: input.thread_watermark,
          eligibilitySha256: input.eligibility_sha256,
          replyCommentId: input.reply_comment_id,
          actorId: input.actor_id,
          actorType: input.actor_type,
          preReadObservedAt: input.pre_read_observed_at,
          postReadObservedAt: input.post_read_observed_at,
          resolvedById: input.resolved_by_id,
          resolvedByType: input.resolved_by_type,
        },
      ),
