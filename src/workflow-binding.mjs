...
export async function readWorkflowBinding(storeRoot, workflowId) {
  ...
  try {
    ...
    if (
      ...
      !isFullSha(workflow.current_head_sha)
    ) {
    ...
    for (const record of addressedFindings) {
      ...
      if (
        ...
        !isFullSha(findingsReview.reviewed_head_sha) ||
        !Array.isArray(record.addressed_by) ||
        record.addressed_by.length === 0 ||
        record.addressed_by.some((sha) => !isFullSha(sha))
      ) {
    ...
    for (const [index, unresolve] of threadUnresolutions.entries()) {
      if (
        ...
        !isFullSha(unresolve.findings_review?.reviewed_head_sha)
      ) {
