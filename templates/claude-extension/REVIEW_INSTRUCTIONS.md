...
2. Call `open_review`.
3. Read the complete patch with `read_review_artifact`; continue from
   `next_offset` until it is null.
4. Inspect relevant source with `read_snapshot_file` and `search_snapshot`.
5. Review correctness, regressions, security, compatibility, error handling,
   ...
   maintenance risk.
6. On round one, call `submit_review`. Use an empty findings array only when no
   actionable issue remains.
7. On round two, decide every previous finding:
...
8. Report new findings separately. Any open or new finding in round two sends
