...
v0.15.3. It contains the packaged server runtime, separate author and reviewer
...
1. Check out the exact `v0.15.3` tag, run `npm ci`, `npm test`, `npm run build`,
   and `npm run verify:build`. Keep the resulting
   `dist/review-bridge-v0.15.3/hermes-integration` directory in place.
...
3. Render both snippets before installing them:
   - `__REVIEW_BRIDGE_RELEASE_PATH__` → the absolute path of the versioned
     ...
     `review-bridge-v0.15.3/hermes-integration`.
...
same absolute `REVIEW_BRIDGE_HOME` and the same absolute v0.15.3 integration
path.
