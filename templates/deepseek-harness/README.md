...
Bridge v0.14.1. It contains the packaged server runtime, separate author and
...
1. Check out the exact `v0.14.1` tag, run `npm ci`, `npm test`, `npm run build`,
   ...
   `dist/review-bridge-v0.14.1/deepseek-harness` directory in place.
...
3. Render both snippets before installing them:
   - `__REVIEW_BRIDGE_RELEASE_PATH__` → the absolute path of the versioned
     ...
     `review-bridge-v0.14.1/deepseek-harness`.
